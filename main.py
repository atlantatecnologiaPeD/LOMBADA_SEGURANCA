...
PORTA_CONVERSOR_RASP = '/dev/ttyUSB0'
...
def sendVel(vel, crc16_alta, crc16_baixa):
    ser_Rasp.write(serial.to_bytes(
        [CABECALHO, COMANDO, ID_FAIXA, vel, crc16_alta, crc16_baixa, RODAPE]))
...
def sensorData():
    ...
    while True:
                    ...
                    try:
                        ...
                        if objeto_detectado:
                            if 'unit' in dict_json and dict_json['unit'] == 'kmph':
                                magnitudes_velocidade.append(
                                    int(dict_json['magnitude']))
                                velocidades_objeto.append(
                                    int(dict_json['speed']))

                            if 'unit' in dict_json and dict_json['unit'] == 'm':
                                magnitudes_distancia.append(
                                    int(dict_json['magnitude']))
                                distancias_objeto.append(
                                    int(dict_json['range']))
                        ...
                        if 'DetectedObject' in dict_json:
                            if dict_json['DetectedObject'] == 'Present':
                                ...
                                dados_json_entrada_saida = json.dumps(
                                    dados_json_entrada_saida) + "\n"
                                try:
                                    clientSocket.send(
                                        dados_json_entrada_saida.encode('utf8'))
                                ...
                                if len(velocidades_objeto) == 0:
                                    ...
                                    velocidade_media = int(
                                        np.around(np.mean(velocidades_objeto)))
                                    magnitude_velocidade_media = np.around(
                                        np.mean(magnitudes_velocidade))

                                # Calculos de distancia e sua magnitude
                                    distancia_media = np.around(
                                        np.mean(distancias_objeto))
                                    magnitude_distancia_media = np.around(
                                        np.mean(magnitudes_distancia))
                                ...
                                dados_json_socket = json.dumps(
                                    dados_json_socket) + "\n"

                                try:
                                    # Envio de dados para o display
                                    msb_CRC_hex, lsb_CRC_hex = displayProcotolo(
                                        velocidade_media)
                                    if flag_envia_display_velocidade:
                                        sendVel(velocidade_media,
                                                msb_CRC_hex, lsb_CRC_hex)
        ...
        if flag_erro:
            ...
            # Convertendo o Json em Str e add o paragrafo para ser enviado via socket
            dados_json_error_socket = json.dumps(
                dados_json_error_socket) + "\n"
